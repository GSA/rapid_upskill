...
|`long-context`|A model that accepts a very large amount of text in one request|A model with a large window|Split by heading; hand off to a new session|
...
*Table: our reading of the reference implementation's documentation; not measured (as of September 2026).*
...
- Stage 3: agents write reports; supply your own checking script, because the reference implementation's is out of date.
- Stage 4: the tutor is prompts and knowledge files, with no code.
- Stage 5: agents write test questions in a fixed JSON format; scripts handle delivery.
- Certification alignment: agents built rating tables with no code; web use covers resource research and link checks.

## Where people replace tools

- **Blocked downloads.** A person downloads paywalled documents by hand; the reference implementation's rule is no accounts and no scraping.
...
- **[Certification outline](glossary.md#certification-outline).** Published by the certifying body; alignment maps to it.
- **Reviewers.** Subject-matter experts with sign-off.
- **A file store with history.** Version control or back-ups, so a lost session costs one batch.
- **[Rate limits](glossary.md#rate-limit) and cost.** Pace calls to services; use a cheaper model for high-volume steps, not judgment steps.
- **Credentials.** Keep keys for quiz-form, storage and image services out of repositories.
- **Tutor-platform size limits.** Check caps on file count, file size and instruction length before Stage 4.
...
- **Unchecked AI output.** Expect [hallucination](glossary.md#hallucination) and invented citations; check output and its [provenance](glossary.md#provenance).
- **Stale scripts that overwrite results.** Read a script, back up files and dry-run it before writing.
