---
title: "X-CA-01 Schedule check"
parent: "Certification alignment scripts"
grand_parent: "Script library"
nav_order: 1
generated: true
---
<!-- GENERATED by scripts/site/sync.py from scripts/ca/schedule_check.py. Do not edit. -->
{% raw %}

# X-CA-01 Schedule check

| Field | Value |
|---|---|
| Purpose | Check a study-schedule JSON file for the errors and warnings described below: that every chapter's reading and active-learning minutes sum to its own stated total, and that no lower importance-tier chapter is given strictly more total time than a higher-tier chapter. |
| Usage | python3 scripts/ca/schedule\_check.py --help In a shell: python3 scripts/ca/schedule\_check.py SCHEDULE.json |
| Dependencies | stdlib |
| Writes files | no |
| License | CC0-1.0 |
| Inputs | A schedule JSON file: a list of chapter objects, each with chapter (a number), importance\_tier ("high", "medium" or "low"), reading\_minutes, active\_minutes and total\_minutes. |
| Outputs | One line per error or warning, then a summary line, all printed to standard output. |
| Used in | [Certification alignment](../../certification-alignment.md) |
| Source | [scripts/ca/schedule\_check.py](https://github.com/GSA/rapid_upskill/blob/master/scripts/ca/schedule_check.py) |
