"""Checks for scripts/sample_data/git_basics_stage3/, the Stage 3 sample data.
...
Purpose: verify the folder's file set, hygiene (ASCII, no local paths, no
    ...
    which read a file outside this folder (claim_source_check.py,
    citation_fidelity_check.py) still behave as documented against the real
    running example's admitted sources.
Usage: python3 -B scripts/tests/test_stage3_sample_data.py
Dependencies: stdlib
Writes files: no
License: CC0-1.0
"""
...
DATA = ROOT / "scripts" / "sample_data" / "git_basics_stage3"
...
class CrossFolderScriptTests(unittest.TestCase):
    """claim_source_check.py and citation_fidelity_check.py both also read
    the running example's real, already-published sources/ folder; confirm
    both still behave as their own pages document."""

    def test_claim_source_check_on_the_running_example(self) -> None:
        proc = run(
            [
                "scripts/s3/claim_source_check.py",
                "scripts/sample_data/git_basics_stage3/claims/claims.json",
                str(SOURCES_DIR.relative_to(ROOT)),
            ]
        )
        self.assertEqual(proc.returncode, 1, proc.stdout + proc.stderr)
        self.assertIn(
            'gap: claim "C4" cites "SRC-009", no such source is admitted', proc.stdout
        )
        self.assertIn("claims=4 gaps=1", proc.stdout)

    def test_citation_fidelity_check_on_the_running_example(self) -> None:
        proc = run(
            [
                "scripts/s3/citation_fidelity_check.py",
                "scripts/sample_data/git_basics_stage3/citations/citations.json",
                str(SOURCES_DIR.relative_to(ROOT)),
            ]
        )
        self.assertEqual(proc.returncode, 1, proc.stdout + proc.stderr)
        self.assertIn("fail quote-C5: not found verbatim in SRC-001.md", proc.stdout)
        self.assertIn("quotes=5 pass=4 fail=1", proc.stdout)
